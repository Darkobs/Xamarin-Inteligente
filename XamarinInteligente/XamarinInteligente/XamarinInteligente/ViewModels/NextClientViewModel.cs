...
namespace XamarinInteligente.ViewModels
{
    class NextClientViewModel : BaseViewModel
    {
        ...
        public Command ShowRouteCommand => new Command(async () =>
        {
            try
            {
                List<Position> completeRoute = new List<Position>();
                ...
                if (placemark != null)
                {
                    var origin = $"{placemark.FeatureName} {placemark.SubLocality} {placemark.Locality}, {placemark.CountryCode}, {placemark.PostalCode}";

                    string destination = "Av. Vasco de Quiroga 1700, Santa Fe, Bosques de las Lomas, 05120 Ciudad de México, CDXM";

                    //var directions = await Services.GoogleDirectionsApi.DirectionsApi.GetDirectionsResponse(origin, destination);

                    var directions = await Services.GoogleDirectionsApi.DirectionsApi.GetDirectionsResponse($"{currentLocation.Latitude},{currentLocation.Longitude}", destination);

                    var steps = directions?.routes.FirstOrDefault()?.legs.First().steps;

                    foreach (var step in steps)
                    {
                        var points = Services.GoogleDirectionsApi.GooglePoints.Decode(step.polyline.points);

                        foreach(var point in points)
                        {
                            completeRoute.Add(new Position(point.Latitude, point.Longitude));
                        }
                    }
                }
                Route = completeRoute;
